...
class RetailStore:
    ...
    def _initialize_ucp_metadata(self):
        with open(os.path.join(os.path.dirname(__file__), "data/ucp.json"), "r") as f:
    ...
    def search_products(self, query: str) -> ProductResults:
        ...
        keywords = [k for k in query.lower().split() if k not in stop_words]

        # Simple plural handling
        expanded_keywords = set(keywords)
        for k in keywords:
            if k.endswith("s"):
                expanded_keywords.add(k[:-1])
            if k.endswith("es"):
                expanded_keywords.add(k[:-2])
